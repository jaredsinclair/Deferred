...
extension Task.Result {
    ...
    private enum InitializerError: Error {
        case invalidInput
    }
    ...
    public init(value: SuccessValue?, error: Error?) {
        ...
        case (nil, let e?):
            self = .failure(e)
        case (nil, nil):
            self = .failure(InitializerError.invalidInput)
        }
    }
}
...
@available(swift, obsoleted: 4, message: "Use Task.Result instead.")
public typealias TaskResult<SuccessValue> = Task<SuccessValue>.Result

extension Task.Result {

    @available(swift, obsoleted: 4, renamed: "SuccessValue")
